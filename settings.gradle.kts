...
rootProject.name = "collector-placeholder"
...
dependencyResolutionManagement {
    ...
    repositories {
        ...
        // and its hash verification chokes while writing verification-metadata.xml
        // — sometimes writing an incomplete file (gradle/gradle#32739, #26803, open).
...
val vaniaCoreDir: File = providers.gradleProperty("vaniaCore.dir").orNull?.let { file(it) } ?: run {
    ...
    if (!dir.resolve(".git").exists()) {
        ...
        // TODAY, whether it's a tag or a branch. Without network, we keep the
        // clone as long as it's at the right ref — otherwise we refuse, rather
